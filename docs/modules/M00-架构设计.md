...
**大白话**：盖楼前先画图纸——没有图纸就动工，盖到三楼发现电梯井没留、承重墙错位，返工的代价是推倒重来。本文档就是你项目的**总平面图 + 施工规范**：后面 22 个模块的每一行代码，都能在这张图上找到"盖在第几层、和谁相邻、水电怎么走"。你以后每次写代码前迷路了，回来翻这张图——**"我在哪、我为什么在这"**（§9 映射表逐一对号）。
...
## 0.5 ★ S0 骨架施工清单（半天可执行，M00 的"作业答案版"）

> 按顺序执行，每步带验证。完成后仓库从"文档集"变成"可开发的工程"。

| # | 动作 | 命令/产物 | 验证 |
|---|---|---|---|
| 1 | 建根目录骨架 | `docs/ config/ backend/ frontend/ training/ benchmarks/ lab/ deploy/` | `tree` 与 §2 图一致 |
| 2 | 打包核心包 | `cd backend && uv init` → pyproject.toml（name=agent-godot，deps: httpx/numpy/pytest/pydantic）+ `agent_godot/__init__.py` + `core/__init__.py` | `uv run python -c "import agent_godot; print('ok')"` |
| 3 | 归位实验区 | 已有实验脚本移入 `lab/m01/`（M01 的四个）、`lab/m02/`（raw_sse） | 脚本仍可跑 |
| 4 | 配置面 | `config/models.yaml`（README 示例改成本机模型）+ `backend/.env.example` | yaml 可被 python 加载 |
| 5 | CI 空管道 | `.github/workflows/ci.yml`：lint（ruff）+ test（pytest 空）+ type（mypy 空） | CI 绿灯 |
| 6 | 首次提交 | `git add -A && git commit -m "S0: skeleton"` | git log 第一条 |
| 7 | 写 ADR-0001 | `docs/adr/ADR-0001-核心包纯库化.md` | §13 作业 |

**注意**：M02 开工前只建 `agent_godot/core/` 的文件——目录树里其他空目录**不要预建**（空目录 git 不跟踪、IDE 还会诱惑你提前填肉），各模块到岗时按自己的 §0.5 清单再建。
...
**三条铁律**（面试常问"怎么防架构腐烂"）——用**公司组织**打比方：

1. **核心包是纯库**：`agent_godot` 不 import FastAPI/React 概念，不感知 HTTP 与租户——它只认"会话 ID + 配置对象"。=**研发部门不接待客户**：客户的事（HTTP/认证/租户）全由前台（app）处理，到研发手里已经是干净的任务单。正因如此，CLI（MI-1）、应用端（MI-7）、离线评估（GodotBench）三个消费者复用同一套 Runtime——同一批工程师服务三条产品线。
2. **应用端是外壳**：认证/限流/审计/配额全部在 `app/` 层完成，穿透到核心包时已变成干净的领域调用。=**前台礼仪不能污染研发规范**：外壳可以随时换（Web 换 CLI、换 gRPC），车间不随之改动。
3. **一切皆插件**（DSH 思想）：模型（models.yaml）、MCP 服务器（mcp.yaml）、权限规则（permissions.yaml）、Hooks、Skills、Commands 全部注册制——新增不改核心，只加插件。=**新员工入职不改编制表**：挂上工牌（注册）就能干活，核心流程零改动。
...
│  ├─ settings.yaml                   #   端口/路径/预算/开关/阈值（M23）
│  └─ prompts.yaml                    #   LLM 提示词模板（M23，缺省回落代码默认值）
├─ backend/
...
│  │  │  └─ fusion.py                 #   向量+图谱融合查询
│  │  ├─ voice/                       # —— 语音（M16）
│  │  │  ├─ stt.py                    #   faster-whisper 接入（离线整段）
│  │  │  ├─ stream_asr.py             #   流式转写（partial/final 增量）
│  │  │  ├─ tts.py                    #   流式合成（分句逐块）
│  │  │  ├─ realtime.py               #   全双工会话状态机（重叠流水线+打断）
│  │  │  ├─ vad.py                    #   VAD 分段
...
**大白话**：这就是**大楼的楼层规定**——地基（core）不许依赖任何上层（地基打好了才盖楼）；能力包是同层住户，**不许互相串门**（memory 想用 RAG，不能直接去 rag 家拿——找物业（agent 层）协调，或发通知（事件）让 rag 送来）；所有出楼办事（数据库）必须走同一个传达室（db/ports.py）——传达室换人（SQLite→MySQL）住户无感。防的是什么病：**能力包横向 import 一旦开了口子，半年后你画不出依赖图**——每个包都"顺便"用了隔壁的东西，牵一发动全身，这就是"架构腐烂"的起点。
...
| M16 STT 语音 | S11 | `voice/`（含 stream_asr/tts/realtime） | 录音→诊断报告；`voice chat` 实时对话首音<1.5s 可打断 |
...
| M23 配置外置化 | 横切 | `config/settings.yaml` `prompts.yaml` + `core/settings.py`（随各模块渐进，见 M23 §5） | 删 yaml 照常跑；改 prompt 不改代码 |
...
**1. 为什么核心包必须是纯库？**
答：三个硬理由：①**三消费者复用**——CLI（MI-1）、Web 应用端（MI-7）、GodotBench 评估（M22）跑的是同一套 Runtime：一处修 bug 三处受益；如果核心包感知了 FastAPI，评估器就要起 Web 服务才能测 Agent，荒谬；②**测试便宜**——纯库的测试直接构造对象调用，不起端口不造 HTTP 请求（快两个数量级）；③**演进自由**——外壳技术栈可换（Web 换 gRPC、加桌面端），核心不动。判断一个包是否纯库的试金石：**它能不能被一个 10 行的脚本完整驱动**。

**2. 能力包之间禁止横向 import，那 memory 需要 RAG 检索怎么办？**
答：两条合法路径：①**由 agent 层串线**——编排者（loop/orchestrator）先调 rag 拿到结果，再把结果作为参数传给 memory（能力包只暴露"给入参返回出参"的纯函数式接口，协作逻辑上移到编排层）；②**事件解耦**——memory 订阅"会话结束"事件自行抽取，不直接调任何人（M08 的实际做法）。禁止横向 import 的收益：依赖图保持树状（可画、可测、可替换），memory 想下线/换实现不波及 rag。代价：编排层稍厚——这是值得付的税。

**3. 端口-适配器在本项目哪里用了？换 MySQL 成 PostgreSQL 改几个文件？**
答：用在 db/ports.py（仓储协议）与 core/llm.py（LLM 协议）两处。db 侧：核心包只 import ports（协议定义），实现（sqlite_impl/mysql_impl）由入口注入——换 PostgreSQL：新增一个 `postgres_impl.py` 实现 ports 协议 + 改配置注入处（cli.py/app 各一行）+ 迁移脚本，**核心包零改动**。llm 侧同理：换模型厂商=新增 Adapter+注册（M02 的注册表），Loop 一行不改。这就是"面向协议编程"的兑现：**协议是桩，实现是插件**。

**4. SSE 相比 WebSocket 在本场景为什么更合适？什么场景必须换 WS？**
答：本场景（Agent 回答流）：通信本质是**单向服务端推送**（用户发一条、看模型逐字流+工具卡）——SSE 是 HTTP/1.1 原生（企业代理/防火墙友好）、自带重连语义（Last-Event-ID）、每帧带 id 天然支持补发，实现简单（fetch 流）。必须换 WS 的场景：**双向实时高频**——协同编辑（光标/编辑流毫秒级双向）、语音全双工（边说边转写边合成）、多人白板。判断口诀：客户端只收不发选 SSE，双向都是"主角"才上 WS——用错方向的代价是复杂度（WS 的代理配置/断线语义/心跳都要自己造）。

**5. 前端为什么用 fetch+ReadableStream 手写 SSE 而不用 EventSource？**
答：EventSource 三不满足：①只能 GET——发消息要 POST body（指令+模式+上下文引用）；②不能带 Authorization 头——JWT 认证过不去；③不能设 content-type。手写 fetch 流补齐全部，且掌控重连策略（指数退避+Last-Event-ID 头）与 401/429 的分流处理（401 静默刷新重试、429 退避提示）。附赠收益：手写一遍帧解析（\n\n 分帧+跨 chunk 缓冲），所有流式协议（WS 消息帧、HTTP2 流）触类旁通。

**6. 训练与服务为什么物理隔离？如果训练要在线学习呢？**
答：隔离的三个理由：①**故障隔离**——训练显存爆炸/OOM 崩溃只烧训练容器，在线服务无感（生产铁律：训练失败绝不影响在线）；②**依赖隔离**——训练栈（torch/verl/CUDA 特定版本）与服务栈（FastAPI/轻依赖）版本要求经常冲突，同环境=依赖地狱；③**资源弹性**——训练要 GPU 池按需起停，服务要常驻，伸缩模型完全不同。在线学习（如有需求）的正确形态：训练侧持续产新 adapter → 评估（GodotBench）→ **合格才发布**到服务的模型注册表（models.yaml 灰度）——学习的"在线"体现在发布管道快，而不是训练进程嵌进服务进程。

**7. CLI 先行的演进路线，相比"直接写 Web"省了什么、赚了什么？**
答：省的：①前端整套工作量（M20 的 15 文件）推迟 15 周——期间核心能力全部经 CLI 验证，**反馈回路从"天级"压到"分钟级"**；②Web 框架选型的时间压力（技术选型可以最后做，用最新知）；③接口设计错误的重写成本。赚的：①**接口被最薄的消费者检验**——CLI 只需要"函数调用"级接口，任何过度设计（为 HTTP 加的状态）在 CLI 阶段就会被发现砍掉；②Web 版启动时，核心包 API 已稳定 15 周——M19 的 AgentService 桥接层写得飞快（接口不再变动）；③面试素材：这条路线本身就是"先验证内核再铺外壳"的架构方法论。风险提示：CLI 阶段要忍住"顺手加点 CLI 专属逻辑"——那会污染纯库。

**8. 多租户数据隔离三方案怎么选？**
答：三方案：独立库/租户（隔离最强，成本运维 ×N，大客户合规场景）、独立 schema/租户（中间态，迁移 ×N）、共享表+tenant_id（成本最低，靠查询纪律保证隔离）。本项目选共享表的推理：产品定位个人/小团队（租户量可控）、成本最优先、noisy neighbor 由配额中间件兜住。判断维度四个：合规要求（医疗金融大客户要物理隔离）、租户量级（百级以下共享表性价比无敌）、成本预算、演进路径（共享→大客户独立=混合模式的管道要预留 tenant 路由层）。关键工程点：共享表的命门是**每条查询带 tenant 过滤**——三道防线（ORM 基类强制注入/审计钩子/双租户测试）在 M19 展开。

**9. models.yaml 热加载如何避免运行中会话撕裂？**
答：撕裂场景：会话正在用 deepseek-chat 跑第 10 轮，热加载把该模型配置改了（或删了）——第 11 轮构造请求时读到新配置，行为突变或报"模型不存在"。防撕裂三件套：①**配置版本化**——加载时生成 config_version，会话开始时快照绑定该版本，整个会话期间用快照（模型/参数一致性，像 M09 事件溯源的确定性纪律）；②**引用计数优雅退出**——热加载不删除"仍在被会话引用"的旧配置，等最后一个引用者结束才回收（类似热更新 DLL 的做法）；③**变更通知**——配置重载后发事件，UI 提示"配置已更新，新会话生效"（用户预期管理）。一句话：**热加载的是"新会话的世界"，旧会话活在快照里**。

**10. 开放题：Godot 换成 Unity，架构里哪些层零改动、哪些要重写？**
答：零改动层（分层的回报）：core（模型网关——与引擎无关）、agent（Loop/预算/事件流——纯编排）、context/memory/hooks/command/skills（能力层）、permission/session 的骨架（风险分级通用）、app/frontend 的全部骨架（外壳不知道引擎是谁）、training 的方法论（换语料即可）、benchmarks 的框架（换任务与 verifier 内容）。要重写/大改层：tools/godot/*（M06 整个模块——.tscn 解析器换成 .unity YAML 解析器、GDScript→C#、headless 四级校验换成 Unity batchmode 方案且没有秒级语法检查，闭环质量降级——这正是当初 M06 说"Godot 是文本资产+官方 CLI 最优落地"的反面印证）；mcp/servers/godot 同理。核心洞察：**改动的全部集中在"领域工具层"，且被端口（ToolRegistry/FC 协议）圈住**——领域知识封装得越好，迁移成本越低。这是"分层边界"的终极考题答案。
