...
**为什么本模块只有 ReAct？——ReAct 是循环底座，不是"ask 模式专属范式"**。本项目 Sprint 2 的产品只有 ask 模式，ReAct 循环就是它的全部执行内核；后续 craft/plan/multi 三模式**共用同一个循环**，只是挂上不同的"契约配置"（工具权限/审批门/编排形态，M13）。

**★ 先钉死一个易错认知**：范式（ReAct/Reflection/Plan-and-Solve/Multi-Agent）与模式（ask/craft/plan/multi）是**两层正交的概念，不可 1:1 绑定**——

- **模式**是产品层的"人机协作契约"，回答"模型能做什么、要不要问人"（M13 §1.1）
- **范式**是技术层的"执行机制"，回答"循环怎么组织、怎么纠错、怎么规划"（M13 §1.2）

ReAct 之所以在本模块独占一篇，是因为它是**四模式共用的循环底座**，不是因为它是 ask 的专属范式。ask 模式里模型照样在跑 ReAct 多轮检索（搜→看结果→决定再搜什么）；craft 模式内部除了 ReAct 还跑着 Reflection；plan 模式内部更是同时跑着三个范式。详见 M13 §1.3「范式 × 模式矩阵」。

**四大范式在本项目的完整落点**（注意：落点模块 ≠ 所属模式）：

| 经典范式 | 作用（回答什么问题） | 落点模块 | Sprint |
|---|---|---|---|
| **ReAct** | 每步怎么做决策？（循环底座） | **M03（本模块）** → M12 联通检索 → M13 挂四模式 | S2/S8/S9 |
| **Reflection**（客观验证器回路） | 做错了怎么发现并修正？ | M06 领域层落地（headless）→ M13 钩子化 | S4/S9 |
| **Plan-and-Solve**（DAG + re-plan） | 长任务怎么不跑偏？ | M13 §1.5 | S9 |
| **Multi-Agent**（Orchestrator-Worker） | 大任务怎么加速？ | M13 骨架 → M15 完全体 + A2A | S9/S10 |

一句话记住分工：**M03 造发动机（循环），M06/M09/M12 加油路与刹车（验证/权限/检索），M13 换挡（四模式契约配置），M15 加气缸（并行子代理）**——四大范式一个不少，只是按真实产品的节奏分期长出来；而四模式是长在发动机上的四组"驾驶档位"，不对应某个具体范式。
